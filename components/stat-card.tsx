...
export function StatCard({ target, label, suffix = "", isDecimal = false }: { target: number, label: string, suffix?: string, isDecimal?: boolean }) {
  ...
  return (
    ...
    >
      <div className="absolute inset-x-0 bottom-0 h-1 bg-linear-to-r from-transparent via-amber-400 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 shadow-[0_0_10px_rgba(251,191,36,0.5)]"></div>
