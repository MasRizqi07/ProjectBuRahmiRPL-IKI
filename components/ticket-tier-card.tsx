...
export function TicketTierCard({ tier, concertId }: TicketTierCardProps) {
  ...
  return (
    <div className="border border-zinc-700/50 rounded-2xl p-6 bg-zinc-900/50 hover:border-amber-500/30 transition-all duration-300">
      <div className="mb-4">
      ...
      >
        <Button
          className="w-full bg-amber-400 hover:bg-amber-500 text-zinc-950 font-bold transition-all duration-200"
          disabled={!isAvailable}
        >
          {isAvailable ? 'Beli Tiket' : 'Terjual Habis'}
        </Button>
      </Link>
    </div>
  )
