              ...
              {orders.map((order) => (
                <div key={order.id} className="bg-zinc-900 border border-zinc-800 rounded-xl p-6">
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      <h3 className="font-bold text-xl text-white mb-2">{(order.concerts as any)?.title}</h3>
                      <p className="text-zinc-400 mb-1">{(order.ticket_tiers as any)?.name} - {order.quantity} Tiket</p>
